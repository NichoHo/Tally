...
a completely free hosted demo (dashboard on Vercel, backend on Render, Postgres
on Neon), the fraud service is scored synchronously over HTTP instead, so
nothing has to stay running 24/7. The same code drives both: setting
`FRAUD_SCORE_URL` on the gateway switches it on, leaving it unset keeps the
local Kafka path. See [docs/deploy.md](docs/deploy.md).
