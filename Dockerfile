...
# process; it's used only by the free Render deploy, see
# services/ledger/cmd/renderall.
...
RUN CGO_ENABLED=0 GOOS=linux go build -o /out/ledger ./services/ledger/cmd/ledger && \
    ...
    CGO_ENABLED=0 GOOS=linux go build -o /out/renderall ./services/ledger/cmd/renderall
